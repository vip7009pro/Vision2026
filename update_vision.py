...
# Patch Match(Mat, PointDefinition, Mat, PreprocessSettings?)
match_patch = '''
        ...
        {
            // Ignore ShapeModel, go straight to TemplateMatch Pyramids
        ...
        // ShapeBased (Default) fallback logic below
'''
text = re.sub(r'(\s*using var roiGray = EnsureGrayBorrowed\(roi\);\s*)using var templPrep = PreprocessTemplateForMatch\(templateGray, preprocess\);', match_patch + r'using var templPrep = PreprocessTemplateForMatch(templateGray, preprocess);', text)

# Patch MatchWithFixedRotation(Mat, PointDefinition, Mat, double, PreprocessSettings?)
fixed_rot_patch = '''
        using var roiGray = EnsureGrayBorrowed(roi);
...
'''
text = re.sub(r'(\s*using var roiGray = EnsureGrayBorrowed\(roi\);\s*)if \(definition\.ShapeModel is not null', fixed_rot_patch, text)

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(text)
print("Updated Class1.cs")
