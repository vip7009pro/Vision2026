...
- **Khắc Phục Triệt Để Lỗi Ảnh 20Mpx Bị Thu Nhỏ Về 1280x853 Sau Khi Bật Undistort & Thích Ứng Tự Động Độ Phân Giải Hiệu Chuẩn (Task 351)**:
  ...
  - **Phân Tích Nguyên Nhân Gốc Rễ**:
    1. *Nguồn gốc con số 1280x853*: Khi lưu Job, hàm `EnsureTeachImageSavedToCacheAndPreview` ([ToolEditorViewModel.Config.cs](file:///g:/NODEJS/Vision2026/VisionInspectionApp.UI/ViewModels/ToolEditorViewModel.Config.cs#L567)) tạo ra file thumbnail JPEG nén nhẹ `teach_preview.jpg` với cạnh dài nhất `maxDim = 1280`px. Với ảnh 20Mpx ($5472 \times 3648$, tỉ lệ 1.5), kích thước nén ra đúng bằng $1280 \times 853$ ($3648 \times 1280 / 5472 = 853.33$).
    2. *Xóa cache RAM khi bật Checkbox Undistort*: Setter `ImageSource_EnableUndistort` trong [ToolEditorViewModel.ToolPreprocess.cs](file:///g:/NODEJS/Vision2026/VisionInspectionApp.UI/ViewModels/ToolEditorViewModel.ToolPreprocess.cs#L560) gọi `ClearImageSourceCache(def.Name)`, vô tình xóa sạch bản sao ảnh RAW 20Mpx trong bộ nhớ RAM (`_imageSourcePreviewCache`).
    3. *Thứ tự nạp đệm sai ưu tiên trong `TryLoadUrlImageFromDiskCache`*: Do cache RAM bị xóa, hệ thống gọi `LoadImageFromSourceForPreview` nạp lại ảnh từ đĩa. Hàm `TryLoadUrlImageFromDiskCache` lại ưu tiên kiểm tra `teach_preview.jpg` ở bước 2 (đứng trước cả ảnh gốc chất lượng cao `{ProductCode}_teach.png` và URL hash cache). Kết quả: hệ thống nạp đúng file thumbnail $1280 \times 853$, đưa qua Undistort và gán vào `_sharedImage`.
    4. *Độ phân giải hiệu chuẩn không tương thích (Calibration Resolution Mismatch)*: Cấu hình `ChessboardCalibrationData` chưa lưu `ImageWidth` và `ImageHeight` lúc calibrate. Khi dùng ma trận $K$ được tính từ ảnh nhỏ (hoặc VGA $640 \times 480$, $cx=320, cy=240$) áp dụng cho ảnh 20Mpx mà không scale ma trận camera, quang tâm và tiêu cự bị lệch hàng nghìn pixel.
...
- **Tá»± Äá»™ng Báº­t/Táº¯t ROI & Crosshair Trong OQC Scanner & Báº£o Tá»“n Tráº¡ng ThÃ¡i Full Screen Khi ÄÃ³ng Cá»­a Sá»• Con (Task 315)**:
  - **Hiá»‡n TÆ°á»£ng & YÃªu Cáº§u**:
    ...
    2. Cá»­a sá»• chÃ­nh Ä‘ang Full Screen, má»Ÿ cá»­a sá»• con rá»“i táº¯t Ä‘i thÃ¬ cá»­a sá»• chÃ­nh bá»‹ co láº¡i diá»‡n tÃ­ch nhá» hÆ¡n Full Screen.
  - **Giáº£i PhÃ¡p Triá»ƒn Khai**:
    1. *OQC Scanner*: Trong `OqcScannerViewModel.cs`, tá»± Ä‘á»™ng gÃ¡n `ShowRois = true` vÃ  `ShowCrosshair = true` khi Live Camera báº­t (má»Ÿ Job tá»« Quáº£n lÃ½ Job, quÃ©t mÃ£ barcode); khi `HandleInspectionCompletedAsync` hoÃ n táº¥t, tá»± Ä‘á»™ng gÃ¡n `ShowRois = false` vÃ  `ShowCrosshair = false` vÃ  cáº­p nháº­t preview overlays sáº¡ch sáº½.
    2. *Báº£o tá»“n Full Screen*: Trong `App.xaml.cs` hÃ m `BringWindowToForeground`, thay tháº¿ lá»‡nh Win32 `SW_RESTORE` (vá»‘n un-maximize cá»­a sá»•) báº±ng `SW_SHOWMAXIMIZED` khi cá»­a sá»• Ä‘ang á»Ÿ tráº¡ng thÃ¡i Maximized, Ä‘áº£m báº£o MainWindow luÃ´n Full Screen á»•n Ä‘á»‹nh tuyá»‡t Ä‘á»‘i.
  - **Kiá»ƒm Thá»­**:
    - Bá»• sung `TestBringWindowToForegroundPreservesMaximized` trong `TestExtractApp/OqcLiveViewOnJobLoadTests.cs` Ä‘áº¡t PASSED.
    - dotnet build VisionInspectionApp.slnx: 0 errors.
    - dotnet run --project TestExtractApp: 100% PASSED.
- **NÃ¢ng Cáº¥p Äá»™ TÆ°Æ¡ng Pháº£n Giao Diá»‡n OCR, Kháº¯c Phá»¥c CÆ¡ Cháº¿ Dáº¡y Chá»¯ 'Line Source' & Há»— Trá»£ Äa Dáº¡ng Model AI ONNX KÃ¨m Cáº£nh BÃ¡o Model Sai Loáº¡i (Task 314)**:
...
- **Tích Hợp Hệ Thống Kịch Bản Hiệu Ứng Nháy Đèn (Lighting Blink Pattern & Scenarios) Khi Mở App, Tắt App và Báo Lỗi NG (Task 298)**:
  - **Yêu Cầu & Bối Cảnh**:
    ...
    2. Cho phép chạy blink pattern trước khi bật mức sáng đã lưu khi mở app (Startup), chạy blink pattern trước khi tắt nguồn đèn khi đóng app (Shutdown), và chạy blink pattern cảnh báo khi kiểm tra sản phẩm có lỗi NG (Inspection NG).
    3. Cung cấp công tắc Bật/Tắt độc lập cho từng sự kiện: Bật app, Tắt app, Kiểm tra hàng NG.
    4. Cho phép chọn pattern riêng cho từng mục.
    5. Quy tắc cú pháp pattern thông minh: Hỗ trợ cú pháp chuỗi phẩy (`L1, ON, 300, L1, OFF, L2, ON, 100, L2, OFF...`), cú pháp nhiều dòng (`ALL OFF; DELAY 150`), macro (`STROBE`, `CHASE`), cấu hình số chu kỳ lặp lại (`RepeatCycles`), và hướng dẫn cú pháp chi tiết trực tiếp trên giao diện để người dùng dễ nắm bắt.
    6. Hỗ trợ nút Chạy Thử (Test Run) và Dừng (Stop) trực tiếp trên UI.
  - **Giải Pháp Kỹ Thuật Đã Triển Khai**:
...
- **Khắc Phục Lỗi NullReferenceException / Race Condition Khi Tắt Ứng Dụng (ReadBridgeTagValueAsync / WriteBridgeTagValueAsync Khi Dispose PLC Bridge) (Task 276)**:
  - **Yêu Cầu & Bối Cảnh**:
    - Khi tắt ứng dụng, xuất hiện ngoại lệ:
      ```text
      System.NullReferenceException: 'Object reference not set to an instance of an object.'
      bridge was null.
      at VisionInspectionApp.Application.PLC.Drivers.MitsubishiMxComponentDriver.ReadBridgeTagValueAsync(MxBridgeClient bridge, PlcTag tag, CancellationToken cancellationToken)
      ```
    - Yêu cầu xử lý triệt để để việc thoát app diễn ra an toàn, không bị crash hoặc văng popup lỗi ngoại lệ.
  - **Phân Tích & Nguyên Nhân Gốc Rễ**:
    - Luồng Polling nền (`PlcPollingEngine`) đang trong chu kỳ lặp gọi `driver.ReadBatchAsync(plcTags)` $\rightarrow$ `ReadBridgeTagValueAsync(_bridgeClient, tag)`.
    - Đồng thời, sự kiện đóng app `App.OnExit` kích hoạt `ShutdownGracefullyAsync` $\rightarrow$ `PlcManagerService.Dispose` $\rightarrow$ `MitsubishiMxComponentDriver.Dispose`, giải phóng `_bridgeClient` và gán `_bridgeClient = null`.
    - Do `_bridgeClient` bị gán `null` giữa chừng trong khi vòng lặp đọc tag chưa kết thúc, `ReadBridgeTagValueAsync` nhận tham số `bridge == null` và gọi `bridge.GetDeviceAsync`, dẫn đến `NullReferenceException`.
  - **Giải Pháp Kỹ Thuật Đã Triển Khai**:
...
- **Khắc Phục Lỗi Kết Nối PLC Bridge 32-bit (MX Component & GX Works 3 Simulation) Do Thiếu RollForward .NET x86 & Tối Ưu Batch IDispatch COM (Task 275)**:
  - **Yêu Cầu & Bối Cảnh**:
    ...
    - Yêu cầu kiểm tra nguyên nhân gốc rễ và xử lý triệt để giúp hệ thống kết nối thành công 100% với MX Component và GX Works 3 Simulation.
  - **Phân Tích & Nguyên Nhân Gốc Rễ**:
...
- **Cấu Hình 2 Cột Riêng Biệt (ProductCode & ProductName) trong Product Browser Query & Tự Động Điền ProductName Vào Tool Editor Khi Gán Job (Task 274)**:
  - **Yêu Cầu & Bối Cảnh**:
    - Trong màn hình Cấu hình OQC & Database (Mục 3: Danh sách sản phẩm / Product Browser Query): Bổ sung 2 ô TextBox phân định rõ ràng:
    ...
    - **Quy định hoạt động**:
      - Gán Job vào sản phẩm trong CSDL sẽ dùng giá trị của **cột `ProductCode`** (`AssignProductJobAsync`).
  ...
  - **Giải Pháp Kỹ Thuật Đã Triển Khai**:
    ...
    2. [OqcSettingsDialog.xaml](file:///g:/NODEJS/Vision2026/VisionInspectionApp.UI/Views/OQC/OqcSettingsDialog.xaml):
       - Tại Mục 3 "Danh sách sản phẩm (Product Browser Query)", bố trí 2 dòng TextBox: "Tên cột mã SP (Code):" và "Tên cột tên SP (Name):".
    3. [OqcScannerViewModel.Settings.cs](file:///g:/NODEJS/Vision2026/VisionInspectionApp.UI/ViewModels/OqcScannerViewModel.Settings.cs):
       ...
       - Cập nhật `ExecuteAssignProductAsync`: Trích xuất `productCode` từ cột cấu hình để gán CSDL, và `productName` từ cột cấu hình để tự động điền và lưu vào ô Mã SP của Tool Editor (`SyncProductCodeToToolEditor`).
    4. [ToolEditorViewModel.Config.cs](file:///g:/NODEJS/Vision2026/VisionInspectionApp.UI/ViewModels/ToolEditorViewModel.Config.cs):
       - Bổ sung phương thức `ApplyAssignedProductCode(string productCode, string jobFilePath)`: Tự động cập nhật `ProductCode = productCode;`, gán `_config.ProductCode = productCode;` và lưu file `.job` bằng `_jobService.SaveJob`.
  - **Kiểm Thử**:
    - Bổ sung bộ test tự động [ProductAssignAndCodeSyncTest.cs](file:///g:/NODEJS/Vision2026/TestExtractApp/ProductAssignAndCodeSyncTest.cs) kiểm tra:
      - Test 1: Serialization / Deserialization của `OqcScannerConfig` với cả 2 cột.
      - Test 2 & 3: Trích xuất chính xác `productCode` cho DB và `productName` cho Tool Editor auto-fill, cùng cơ chế fallback.
      - Test 4: Đóng gói và cập nhật đồng bộ `ProductCode` vào file `.job`.
    - Toàn bộ test suite của dự án: PASSED 100%.
...
- **Hỗ trợ Nhập trực tiếp Địa chỉ PLC không cần thông qua Tên Tag (Task 232)**:
  ...
  - **Kết Quả Triển Khai Chi Tiết**:
    1. **Thuật Toán Tự Động Suy Luận Kiểu Dữ Liệu `InferDataTypeFromAddress`**:
       - Bổ sung hàm nhận diện tiền tố thông minh:
         - Địa chỉ thanh ghi từ (Word/Register) độ ưu tiên cao: `MW`, `IW`, `QW`, `SW`, `ZR`, `TN`, `CN`, `SD`, `3x`, `4x`, `D`, `W`, `R`, `Z` $\rightarrow$ Kiểu `Int16`/`Int32`.
         - Địa chỉ bit (Coil/Discrete) độ ưu tiên cao: `SM`, `TS`, `TC`, `SS`, `SC`, `CS`, `CC`, `DX`, `DY`, `0x`, `1x`, `X`, `Y`, `M`, `L`, `B`, `F`, `S` $\rightarrow$ Kiểu `Bool`.
    2. **Tự Động Tổng Hợp Địa Chỉ Vào Vòng Lặp Polling `GetAllTagsToPoll()`**:
       - `PlcManagerService` tự động quét danh bạ Tags đã khai báo + gom toàn bộ các địa chỉ trực tiếp được cấu hình trong `PlcIndustrialConfig` (Handshake, Heartbeat, Motion, ShiftRegister).
       - Khởi chạy vòng lặp Polling đọc ngầm chu kỳ cao mà không bắt buộc người dùng phải thêm thủ công vào bảng Tags.
    3. **Đọc/Ghi/Bộ Nhớ Đệm 2 Chiều Theo Cả `TagName` Và `Address`**:
       - `ReadTagValueAsync` & `WriteTagValueAsync`: Tự động tạo đối tượng `PlcTag` động với kiểu dữ liệu suy luận cho địa chỉ trực tiếp chưa có trong danh bạ.
       - Cập nhật Cache đồng thời cho cả `plcId:TagName` và `plcId:Address` (và cả theo `plc.Name`).
       - `PlcPollingEngine` & `PlcManagerService` phát sự kiện `OnTagChanged` cho cả `TagName`, `Address`, `plc.Id` và `plc.Name`.
    4. **Đồng Bộ Hoá Toàn Bộ ComboBox Trên UI Hỗ Trợ Nhập & Tìm Kiếm Trực Tiếp**:
       - Nâng cấp tất cả ComboBox trong `PlcManagerWindow.xaml`, `ToolEditorView.xaml` (ImageSource PLC Trigger, PlcRead, PlcWrite, PlcWait, PlcTrigger, Batch Read, Batch Write, Result Transfer) sang dạng Editable ComboBox hai chiều (`IsEditable="True"`, `IsTextSearchEnabled="True"`, `StaysOpenOnEdit="True"`, `PreviewMouseDown="ComboBox_PreviewMouseDown"`).
       - Mở rộng danh sách gợi ý `AvailablePlcAllTagNames` và `AvailablePlcTagNames` gồm cả Tag Name + Address + dải địa chỉ công nghiệp thông dụng (`X0..X15`, `Y0..Y15`, `M0..M20`, `D0..D2000`, `MW100..MW200`).
  - **Biên Dịch & Kiểm Thử Tự Động Hoàn Hảo 100%**:
    - Solution `VisionInspectionApp.slnx` biên dịch **0 Error(s)**.
    - Bộ kiểm thử tự động `TestDirectAddressSupport` trong `TestExtractApp` đạt kết quả **PASS 100%** (4/4 tests: Suy luận kiểu dữ liệu, Tổng hợp Polling tự động, Đường ống Đọc/Ghi/Cache trực tiếp, và Phát tán sự kiện tới các module tiêu thụ như MotionSync).

- **Sửa lỗi ComboBox trong 5 Tab PLC Manager không xổ danh sách Tag/PLC khi bấm chuột (Task 231)**:
  - **Yêu Cầu & Bối Cảnh**:
    - Trong các Tab mới của cửa sổ PLC Manager (Handshake, Watchdog & An toàn, Motion & Encoder, Shift Register), khi người dùng click vào các ComboBox Tag và PLC thì không xổ ra danh sách để chọn.
  - **Nguyên Nhân Gốc Rễ**:
    1. Trong WPF, ComboBox có `IsEditable="True"` gồm 1 vùng TextBox và 1 nút ToggleButton mũi tên bên phải. Khi người dùng click chuột vào vùng chữ (TextBox/TextBlock), WPF chỉ focus vào TextBox để nhập text mà không kích hoạt mở danh sách thả xuống (`IsDropDownOpen`).
    2. Thuộc tính `AvailableTagNames` và `AvailablePlcNames` trước đây trả về `IEnumerable` trực tiếp từ danh bạ Tags của PLC Service. Khi cấu hình chưa có Tag nào hoặc người dùng muốn nhập/chọn nhanh các địa chỉ thanh ghi chuẩn (như `X0`, `Y1`, `D1000`, `M0`), danh sách bị rỗng.
    3. Thiếu binding 2 chiều vào thuộc tính `Text` trên Editable ComboBox (`Text="{Binding ...}"`).
  - **Giải Pháp Triển Khai Chi Tiết**:
    1. **Bổ sung Handler `ComboBox_PreviewMouseDown`**:
       - Trong `PlcManagerWindow.xaml.cs`: Bổ sung hàm `ComboBox_PreviewMouseDown` bắt sự kiện chuột trên TextBox/TextBlock con và lập tức thiết lập `cb.IsDropDownOpen = true`.
       - Gán sự kiện `PreviewMouseDown="ComboBox_PreviewMouseDown"` cùng các thuộc tính `IsTextSearchEnabled="True"`, `StaysOpenOnEdit="True"`, `MaxDropDownHeight="220"` vào toàn bộ ComboBox trong các Tab 2, 3, 4, 5 của `PlcManagerWindow.xaml`.
    2. **Đồng Bộ Hai Chiều `Text` & `SelectedItem`**:
       - Ràng buộc `Text="{Binding ..., Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}"` và `SelectedItem="{Binding ..., Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}"` trên tất cả ComboBox, cho phép vừa gõ tay tự do, vừa chọn một chạm từ danh sách xổ xuống.
    3. **Làm Giàu Bộ Dữ Liệu `AvailableTagNames` & `AvailablePlcNames`**:
       - Chuyển `AvailableTagNames` và `AvailablePlcNames` thành `ObservableCollection<string>` trong `PlcManagerViewModel`.
       - Tự động nạp sẵn các địa chỉ bit/word chuẩn (`X0..X11`, `Y0..Y11`, `D0..D2000`, `M0..M100`, `MW100..MW200`), các Tag công nghiệp tiêu chuẩn (`VisionReady`, `VisionBusy`, `VisionDone`, `PlcAck`, `Heartbeat`, `Fault`, `Encoder`, `Reject`), toàn bộ Tag trong Service (cả `Name` và `Address`), và các Tag hiện tại của `IndustrialConfig`.
  - **Biên Dịch & Kiểm Thử Thành Công 100%**:
    - Solution `VisionInspectionApp.slnx` biên dịch **0 Error(s)**.
    - Toàn bộ unit tests trong `TestExtractApp` đạt kết quả **PASS 100%**.

- **Hoàn thành Triển khai Phase 6: Toàn Diện Hóa UI/UX Cấu Hình PLC Công Nghiệp & Trực Quan Hóa Hàng Đợi (Task 230)**:
  - **Yêu Cầu & Bối Cảnh**:
    1. Đưa toàn bộ cấu hình PLC công nghiệp (Handshake, Heartbeat Watchdog, Motion/Encoder Sync, Shift Register & Trạm Reject) lên giao diện người dùng thay vì hardcode, cho phép cài đặt và chỉnh sửa trực tiếp.
    2. Visualize hóa tình trạng hàng đợi (Queue) xử lý ảnh ngay trên thanh Toolbar của Tool Editor (dưới dạng thanh bar gồm các nấc rời rạc, mỗi nấc đại diện cho 1 con hàng / frame ảnh), căn phải, trên cùng một hàng với các nút `🏷️ Gán Mã - Job`, `💾 Chụp & Lưu Ảnh`.
    3. Đảm bảo 0% suy giảm hiệu năng cho luồng xử lý ảnh liên tục.
  - **Kết Quả Triển Khai Chi Tiết**:
    1. **Mô hình Dữ liệu Cấu hình Toàn Cục `PlcIndustrialConfig`**:
       - Định nghĩa `PlcIndustrialConfig.cs` đóng gói đầy đủ thông số 4 phân hệ: `IndustrialHandshakeConfig`, `PlcHeartbeatConfig`, `PlcMotionConfig`, `PlcShiftRegisterConfig`.
       - Nâng cấp `IPlcManagerService` và `PlcManagerService`: Tự động nạp/lưu cấu hình toàn cục vào `plc_config.json` và phát tán sự kiện `OnIndustrialConfigChanged`.
    2. **Tái Cấu Trúc Giao Diện Quản Lý PLC `PlcManagerWindow.xaml` Thành 5 Tab Chuyên Nghiệp**:
       - *Tab 1: Kết Nối & Danh Bạ Tags*: Quản lý danh sách kết nối PLC và bảng danh bạ biến.
       - *Tab 2: Bắt Tay 24/7 (Handshake)*: Tùy biến toàn bộ các tag `Ready`, `Busy`, `Done`, `Pass`, `NG`, `PLC Ack`, `Timeout (ms)`.
       - *Tab 3: Watchdog & An Toàn*: Tùy biến `Vision Heartbeat`, `PLC Heartbeat`, `Chu kỳ (ms)`, `Timeout (ms)`, `Emergency Stop Tag`, `Interlock Toggle`.
       - *Tab 4: Motion & Encoder*: Tùy biến `Encoder Tag`, `Speed Tag`, `Xung/mm (Pulses/mm)`, `Mm/Pixel`, `Tốc độ định mức (m/phút)`, `Thời gian phơi sáng cơ sở (µs)`.
       - *Tab 5: Shift Register & Loại Bỏ*: Tùy biến `Reject Tag`, `Khoảng cách trạm loại bỏ (mm)`, `Dung sai kích hoạt (mm)`, `Độ rộng xung (ms)`, `Kích hoạt loại bỏ (Toggle)`.
    3. **Thanh Trạng Thái Hàng Đợi (Queue Stepped Bar) Căn Phải Trên Tool Editor Toolbar**:
       - Tích hợp thanh trạng thái LED gồm 8 nấc (Segments) căn phải trên Toolbar `Grid.Row="0"` của `ToolEditorView.xaml`.
       - Mỗi nấc đại diện cho 1 frame / con hàng đang chờ trong `_industrialCameraFrameChannel` (`BoundedChannel<Mat>(8)`).
       - Màu sắc biến đổi thông minh theo tải: Xanh Emerald `#10B981` (Tải nhẹ $\le 2$), Vàng Amber `#F59E0B` (Tải vừa $3-5$), Đỏ Ruby `#EF4444` (Tải cao $\ge 6$).
    ...
    4. **Cửa Sổ Bản Đồ Khuyết Tật Cuộn `RollDefectMapWindow` & Xuất Báo Cáo**:
       - Xây dựng `RollDefectMapViewModel` kết nối dữ liệu từ `RollDefectManager`, `PlcMotionSyncService`, `ShiftRegisterTracker` và `RollReportExporter`.
       - Tạo `RollDefectMapWindow.xaml` nhúng `RollDefectMapControl` với các nút xuất báo cáo JSON, CSV Cut List cho máy xả cuộn, và HTML Certificate in ấn.
       - Bổ sung nút mở nhanh `📜 Bản Đồ Cuộn` trên thanh công cụ Tool Editor và menu item trong `MainWindow.xaml`.
  - **Biên Dịch & Kiểm Thử Tự Động Hoàn Hảo 100%**:
    - Solution `VisionInspectionApp.slnx` biên dịch **0 Error(s)**.
    - Bộ kiểm thử tự động `TestIndustrialUIAndQueueVisualization` trong `TestExtractApp` đạt kết quả **PASS 100%**.
