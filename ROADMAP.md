...
- [x] Task 306: Tá»‘i Æ¯u Hiá»‡u NÄƒng Tool Caliper (ROI First Pipeline) & NÃ¢ng Cáº¥p ToÃ n Diá»‡n Tool Surface Compare (RAM Template Cache, Sub-pixel Auto Align, Confidence Guard, Normalize Lighting, Spatial Edge Tolerance ToÃ n Diá»‡n, Thuáº­t ToÃ¡n Má»›i EdgeCompare).
    ...
    - [x] **Task 311: PhÃ¡t Triá»ƒn & TÃ­ch Há»£p Tool OCR Chuáº©n CÃ´ng Nghiá»‡p (Industrial OCR Tool) Song HÃ nh AI & Non-AI**:
       - Hiá»‡n TÆ°á»£ng & YÃªu Cáº§u NgÆ°á»i DÃ¹ng:
         - NgÆ°á»i dÃ¹ng yÃªu cáº§u phÃ¡t triá»ƒn vÃ  tÃ­ch há»£p Tool OCR chuáº©n cÃ´ng nghiá»‡p hoÃ n chá»‰nh vÃ o VisionInspectionApp vá»›i 2 phiÃªn báº£n song hÃ nh: AI (Deep Learning ONNX) vÃ  Non-AI (Heuristic & Segment Classifier).
       - PhÃ¢n TÃ­ch Ká»¹ Thuáº­t & Kiáº¿n TrÃºc Thiáº¿t Káº¿:
       ...
       - Giáº£i PhÃ¡p Ká»¹ Thuáº­t Ä Ã£ Triá»ƒn Khai:
         ...
         4. *Giao Diá»‡n NgÆ°á»i DÃ¹ng (VisionInspectionApp.UI)*:
            - Táº¡o má»›i ToolEditorViewModel.ToolOcr.cs (phÃ¢n tÃ¡ch mÃ´-Ä‘un hÃ³a).
            - ThÃªm item OCR vÃ o Toolbox, há»— trá»£ FlowCanvas node (Input Image, Output Text), tá»± Ä‘á»™ng Ä‘áº·t tÃªn "OCR".
            - Thiáº¿t káº¿ báº£ng thuá»™c tÃ­nh toÃ n diá»‡n trong ToolEditorView.xaml (Engine, So khá»›p, Nhá»‹ phÃ¢n, Expected Text, Regex, Whitelist, Min Conf, Dot-Matrix connector, Invert, Model AI Path).
            - Hiá»ƒn thá»‹ Ä‘áº¿m sá»‘ lÆ°á»£ng OCR trÃªn LiveCameraView vÃ  InspectionView.
         5. *Kiá»ƒm Thá»­ Tá»± Äá»™ng ToÃ n Diá»‡n (TestExtractApp/OcrDetectorTests.cs)*:
            - Viáº¿t 5 test case toÃ n diá»‡n: Non-AI Recognition & Speed (<7ms), Dot-Matrix Connector, Spec Verification (Regex/Exact/Contains), AI Safe Fallback, Full Pipeline Integration.
            - Káº¿t quáº£: 100% test cases PASSED, toÃ n bá»™ test suite cá»§a giáº£i phÃ¡p hoÃ n thÃ nh thÃ nh cÃ´ng vá»›i Exit Code 0.
       - Kiá»ƒm Thá»­:
         - dotnet build VisionInspectionApp.slnx: 0 errors.
         - dotnet run --project TestExtractApp: 100% PASSED toÃ n bá»™ test suite.

    - [x] **Task 312: Cáº£i Tiáº¿n Robustness & Kháº¯c Phá»¥c Lá»—i Nháº­n Diá»‡n Chá»¯ TrÃªn Ná»n Tá»‘i (Dark Background, Auto-Polarity, Chá»¯ ThÆ°á»ng a-z, TÃ¡ch DÃ­nh NÃ©t & Lá»c Viá»n Báº£ng) Cho Tool OCR**:
        - Hiá»‡n TÆ°á»£ng & YÃªu Cáº§u NgÆ°á»i DÃ¹ng:
          - NgÆ°á»i dÃ¹ng thá»­ nghiá»‡m Tool OCR trÃªn áº£nh thá»±c táº¿ (giao diá»‡n UI dark theme ná»n xÃ¡m tá»‘i, chá»¯ 'Line Mode' mÃ u xÃ¡m sÃ¡ng), cáº¥u hÃ¬nh AnyText + Äáº£o mÃ u, nhÆ°ng khi cháº¡y bÃ¡o lá»—i "KhÃ´ng Ä‘á»c Ä‘Æ°á»£c kÃ½ tá»±" (Found = False, ROI Ä‘á» NG).
        - PhÃ¢n TÃ­ch Ká»¹ Thuáº­t & NguyÃªn NhÃ¢n Gá»‘c Rá»…:
          ...
          3. *MÃ©p dÆ°á»›i Search ROI cháº¡m Ä‘Æ°á»ng káº» ngang viá»n báº£ng*: Thanh viá»n phÃ¢n cÃ¡ch báº£ng táº¡o ra má»™t blob ngang dÃ i, cÃ³ thá»ƒ dÃ­nh vÃ o chÃ¢n chá»¯ náº¿u khÃ´ng Ä‘Æ°á»£c lá»c.
          4. *Chá»¯ cÃ¡i dÃ­nh nÃ©t khi nhá»‹ phÃ¢n hÃ³a*: Khoáº£ng cÃ¡ch háº¹p giá»¯a cÃ¡c chá»¯ cÃ¡i trong tá»« 'Line' hoáº·c 'Mode' khiáº¿n chÃºng dÃ­nh thÃ nh 1 cá»¥m blob lá»›n náº¿u khÃ´ng cÃ³ thuáº­t toÃ¡n tÃ¡ch chá»¯ dÃ­nh nÃ©t.
        - Giáº£i PhÃ¡p Ká»¹ Thuáº­t ÄÃ£ Triá»ƒn Khai:
          1. *CÆ¡ cháº¿ Auto-Polarity thÃ´ng minh (NormalizeTextPolarity)*:
             - Kiá»ƒm tra tá»· lá»‡ pixel tráº¯ng (whiteRatio) vÃ  tá»· lá»‡ pixel tráº¯ng á»Ÿ 4 Ä‘Æ°á»ng viá»n (borderWhiteRatio). VÃ¬ vÄƒn báº£n luÃ´n chá»‰ chiáº¿m 5-40% diá»‡n tÃ­ch vÃ  viá»n mÃ©p ROI háº§u nhÆ° luÃ´n lÃ  ná»n, náº¿u phÃ¡t hiá»‡n ná»n lÃ  mÃ u tráº¯ng (>50%), há»‡ thá»‘ng tá»± Ä‘á»™ng BitwiseNot Ä‘áº£o ngÆ°á»£c vá» chá»¯ tráº¯ng trÃªn ná»n Ä‘en. DÃ¹ ngÆ°á»i dÃ¹ng cÃ³ tÃ­ch nháº§m hay khÃ´ng tÃ­ch "Äáº£o mÃ u", há»‡ thá»‘ng luÃ´n Ä‘Æ°a vá» chuáº©n chá»¯ tráº¯ng ná»n Ä‘en.
    ...
    - [x] **Task 312b: Khắc Phục Triệt Để Lỗi Nhận Diện Sai Thành 'UNWMOJM' Cho Dòng Chữ 'Line Mode' (Smart Whitelist, Phân Đoạn Chiếu Dọc Không Chém Ký Tự, x-Height & Ràng Buộc Hình Học Quadrant, Tối Ưu Tương Quan Unsafe IoU + CCoeffNormed)**:
        ...
        - Phân Tích Kỹ Thuật & Nguyên Nhân Gốc Rễ:
          1. *Whitelist của Job cũ thiếu chữ thường a-z*: Node OCR được tạo từ cấu hình cũ chỉ có '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ-/. :', hoàn toàn không có chữ thường a-z. Khi chạy AnyText, thuật toán bị cấm trả về chữ thường và bắt buộc phải ép mọi ký tự về chữ in hoa.
          2. *Nhát cắt chia đều cơ học w / nChars*: Trong FindSplitBoxesByVerticalProjection, khi gặp blob hơi rộng, thuật toán chia đều cơ học khiến chữ 'n' bị xẻ đôi (nửa trái ghép với 'i' thành 'N', nửa phải thành nét riêng 'W').
          3. *Thiếu phân loại chiều cao Cap-Height vs x-Height*: Các chữ thường lùn 'e', 'o' có chiều cao chỉ 60-70% chữ hoa nhưng bị phóng to chuẩn hóa về 32x24 và so khớp với các chữ in hoa to toàn khung như 'W', 'M' dẫn đến 'e' thành 'W/M'.
          4. *Thiếu ràng buộc hình học góc phần tư (Quadrant Analysis)*: Chữ 'L' có góc trên bên phải rỗng hoàn toàn, khi so khớp với 'U' (có 2 cột trái phải) bị ăn điểm cao hơn 'L'; chữ 'd' có cột cao bên phải và bụng tròn bên trái bị nhầm thành 'J'.
          5. *Template Matching CCoeffNormed bị nhiễu do độ dày nét*: Chỉ dùng hệ số tương quan chuẩn hóa dễ bị lệch khi nét chữ thanh mảnh.
        - Giải Pháp Kỹ Thuật Đã Triển Khai:
          1. *Cơ chế Smart Whitelist tự động*:
             - Tự động bổ sung tất cả ký tự trong ExpectedText vào danh sách được phép nhận diện, không phụ thuộc vào chuỗi Whitelist được lưu trong file Job.
             - Nếu Whitelist trống hoặc là chuỗi mặc định cũ hoặc MatchingMode là AnyText, tự động nạp toàn bộ chữ thường a-z.
             - Bổ sung 2 nút tiện ích trên giao diện ToolEditorView: Tất cả (nạp full ký tự) và Từ mẫu (trích xuất ký tự từ ExpectedText).
          2. *Xóa bỏ nhát cắt chia đều cơ học*:
             - Loại bỏ hoàn toàn segmentW = w / nChars; chỉ cắt phân tách khi có thung lũng (valley) thực sự rõ rệt trong biểu đồ hình chiếu dọc (proj[x] <= 12% * Height), bảo toàn 100% hình dạng chữ 'n', 'i', 'e'.
          3. *Ràng buộc hình học x-Height và Quadrant Analysis*:
             - Phân loại isShortGlyph (chiều cao <= 78% chiều cao tối đa của dòng chữ). Cấm các ký tự thân lùn match với các chữ hoa to toàn khung (W, M, H, N, U, O, D, B...).
             - Ràng buộc Quadrant Top-Right cho chữ 'L': Nếu góc trên bên phải rỗng (< 12% pixel), cấm tuyệt đối match thành 'U' -> Chữ 'L' đạt độ tin cậy 96.9%.
             - Ràng buộc hình học cho chữ 'd' vs 'J', và số '9' vs chữ 'g'.
          4. *Tối ưu hóa tương quan siêu tốc bằng con trỏ Unsafe (Score = 0.5 * CCoeff + 0.5 * IoU)*:
             - Tính toán Intersection-over-Union trực tiếp trên mảng byte thông qua con trỏ charNorm.DataPointer và templateMat.DataPointer, không cấp phát thêm Mat trong vòng lặp.
             - Thời gian so khớp hot-run cực nhanh chỉ 13ms, không tốn tài nguyên GC.
        - Kiểm Thử:
          - Đã cập nhật TestUserCaseLineModeDarkBackground trong TestExtractApp: Nhận diện chính xác 100% từng chữ cái: 'L' (96.9%), 'i' (83.6%), 'n' (83.4%), 'e' (81.9%), 'M' (87.0%), 'o' (85.8%), 'd' (80.5%), 'e' (81.1%) -> Kết quả: 'Line Mode', Avg Confidence: 85.0%.
          - dotnet build VisionInspectionApp.slnx: 0 errors.
          - dotnet run --project TestExtractApp: 100% PASSED toàn bộ test suite.

    - [x] **Task 313: TÃ­nh NÄƒng Dáº¡y KÃ½ Tá»± Máº«u Chuáº©n Hikrobot MVS (Character Font Training) & Náº¡p Model ONNX Linh Hoáº¡t Cho Tool OCR**:
        - Hiá»‡n TÆ°á»£ng & YÃªu Cáº§u NgÆ°á»i DÃ¹ng:
          - NgÆ°á»i dÃ¹ng thá»­ nghiá»‡m nháº­n diá»‡n ra chuá»—i "LineMade" (68%) do chá»¯ 'o' bá»‹ Ä‘á»c nháº§m thÃ nh 'a'. NgÆ°á»i dÃ¹ng nháº­n xÃ©t: cÃ¡c pháº§n má»m vision chuyÃªn nghiá»‡p nhÆ° MVS cho phÃ©p nháº­n diá»‡n vÃ  há»c tá»«ng chá»¯; Ä‘á»“ng thá»i há»i táº£i model ONNX á»Ÿ Ä‘Ã¢u Ä‘á»ƒ sá»­ dá»¥ng.
        - PhÃ¢n TÃ­ch Ká»¹ Thuáº­t & Giáº£i PhÃ¡p:
          1. *TÃ­nh NÄƒng Dáº¡y KÃ½ Tá»± Máº«u (Character Font Training chuáº©n Hikrobot MVS)*:
             ...
             - ThÃªm hÃ m `OcrDetector.TeachCharacters`: Tá»± Ä‘á»™ng cáº¯t cÃ¡c Ã´ kÃ½ tá»± tá»« Search ROI áº£nh hiá»‡n táº¡i, gÃ¡n nhÃ£n theo Chuá»—i máº«u (ExpectedText) vÃ  mÃ£ hÃ³a áº£nh nhá»‹ phÃ¢n máº«u $24 \times 32$ thÃ nh Base64 lÆ°u vÃ o Job.
             - NÃ¢ng cáº¥p `OcrDetector.RunNonAiOcr`: Giáº£i mÃ£ `TrainedCharacters` vÃ  Æ°u tiÃªn so khá»›p sá»‘ 1 vá»›i táº­p kÃ½ tá»± Ä‘Ã£ dáº¡y trÆ°á»›c. Äá»™ tÆ°Æ¡ng quan $\ge 0.65$ Ä‘Æ°á»£c boost Ä‘iá»ƒm tin cáº­y lÃªn 90% - 100%, triá»‡t tiÃªu hoÃ n toÃ n sai sá»‘ cá»§a font chá»¯ thá»±c táº¿.
    ...
    - [x] **Task 323: Khắc Phục Lỗi Cài Đặt Gói Cập Nhật OTA Tại Máy Vision Client (File Lock VisionUpdater.dll), Cơ Chế Retry Đổi Tên NTFS, Ghi Nhật Ký Lỗi updater_error.log Chi Tiết & Banner Cảnh Báo Trên UI**:
        - Phát hiện và xử lý triệt để nguyên nhân gốc rễ: Trong .NET 8 (CoreCLR), `VisionUpdater.exe` là native host nạp và chạy `VisionUpdater.dll`. Gói zip phát hành trước đó chứa cả `VisionUpdater.dll` nên khi `VisionUpdater.exe` đang chạy để giải nén đè lên thư mục app, nó cố gắng giải nén đè lên chính `VisionUpdater.dll` đang chạy trong bộ nhớ RAM, dẫn tới lỗi Windows `System.IO.IOException: The process cannot access the file ... because it is being used by another process`.
        - Cơ chế cũ rollback backup, chỉ hiển thị thông báo lỗi trên TextBlock trong vòng 2.5 giây rồi tự thoát mà không ghi file log, khiến người dùng không biết lý do.
        - Giải pháp đã triển khai:
          1. Trong `VisionInspectionApp.Updater/UpdaterWindow.xaml.cs`: Bổ sung kiểm tra bỏ qua mọi tệp `VisionUpdater*` khi giải nén và khi sao lưu (`entry.Name.StartsWith("VisionUpdater")`); cưỡng chế dừng sạch tiến trình `VisionInspectionApp.UI` và `VisionInspectionApp.LightingServer` còn sót; thêm vòng lặp retry 6 lần kèm fallback đổi tên file cũ `.old_{uuid}` (tận dụng tính năng của Windows NTFS cho phép rename file đang mở); ghi log lỗi chi tiết ra tệp `updater_error.log` ở cả thư mục ứng dụng và thư mục backup; hiển thị hộp thoại cảnh báo `MessageBox.Show` dừng màn hình cho người dùng đọc rõ thông tin lỗi thay vì tự thoát sau 2.5s; truyền cờ `--update-failed` khi restart.
    - [x] **Task 324: Khắc Phục Lỗi Ứng Dụng Không Tự Động Khởi Động Lại Sau Khi Cập Nhật Thành Công (Windows Argument Trailing Slash Escaping Bug & Fallback Restart)**:
        - Hiện tượng: Ứng dụng client đã cập nhật phiên bản mới thành công 100%, tuy nhiên `VisionUpdater` tự thoát mà không thấy tự động bật lại ứng dụng Vision chính.
        - Nguyên nhân gốc rễ:
          1. Trong Windows CLI parser (`CommandLineToArgvW` và .NET `Environment.GetCommandLineArgs()`), ký tự gạch chéo ngược `\` đứng ngay trước dấu ngoặc kép `\"` được xem là ký tự escape (thoát) cho dấu ngoặc kép.
          2. Trong `OtaUpdateService.cs`, `appDir = AppDomain.CurrentDomain.BaseDirectory` luôn kết thúc bằng dấu `\` (ví dụ `C:\App\`). Khi truyền chuỗi tham số `--target "{appDir}" --restart "{currentExe}"`, chuỗi trở thành `--target "C:\App\" --restart "C:\App\VisionInspectionApp.UI.exe"`.
          3. Windows parser hiểu `\"` là ký tự `"` nằm trong giá trị của `--target`, làm cho giá trị `--target` nuốt trọn cả chuỗi `--restart "C:\App\VisionInspectionApp.UI.exe"`. Kết quả là biến `_restartExePath` trong `VisionUpdater` hoàn toàn rỗng (`""`), và updater bỏ qua bước khởi động lại rồi thoát.
        - Giải pháp triển khai:
          1. Trong `OtaUpdateService.cs`: Chuẩn hóa `safeAppDir`, `safeZipPath`, `safeExePath` bằng `TrimEnd('\\', '/')` để triệt tiêu hoàn toàn trailing slash trước dấu ngoặc kép. Đồng thời thiết lập `WorkingDirectory = safeAppDir` khi khởi động updater.
          2. Trong `VisionInspectionApp.Updater/UpdaterWindow.xaml.cs`:
             - `ParseCommandLineArgs`: Tự động nhận diện và bóc tách chuỗi `--restart` nếu bị Windows gộp vào `--target`.
             - `ResolveRestartExecutablePath`: Cơ chế tự động tìm kiếm file thực thi chính (`VisionInspectionApp.UI.exe`) hoặc các file .exe khả dĩ trong thư mục cài đặt nếu tham số `--restart` bị thiếu hoặc sai đường dẫn.
             - `RestartMainApplication`: Thiết lập `WorkingDirectory` chuẩn xác cho tiến trình mới để nạp đúng cấu hình; bổ sung cơ chế fallback `UseShellExecute = false` nếu quyền bảo mật Windows hạn chế; thêm độ trễ đệm trước khi updater thoát để tiến trình chính kịp nạp vào RAM; hiển thị hộp thoại nhắc nhở nếu việc tự động khởi chạy bị chặn.
        - Kết quả: Đạt 100% PASSED toàn bộ bộ test trong `TestExtractApp`, giải pháp biên dịch 0 lỗi Release.
    - [x] **Task 325: Biên Soạn Bộ Tài Liệu Kỹ Thuật Đào Tạo Kỹ Sư Vision, Bản SOP Thao Tác Kiểm Tra Lấy Mẫu OQC & Tích Hợp Trung Tâm Tài Liệu Trực Tiếp Trong Ứng Dụng**:
        - **Hiện tượng & Nhu cầu**: Cần một bộ tài liệu đào tạo chuyên sâu dành cho Kỹ sư Vision làm chủ phần mềm CMS VINA VISION SYSTEM, một bản Quy trình Thao tác Chuẩn (SOP) kiểm tra lấy mẫu sản phẩm dành cho công nhân OQC kèm hình ảnh minh họa công nghiệp trực quan, sống động; đồng thời tích hợp Trung tâm Tài liệu (Documentation Viewer) ngay trong menu Trợ Giúp để người dùng luôn có thể mở tài liệu trực tiếp từ app (Tài liệu là một phần của ứng dụng).
        - **Giải pháp triển khai**:
          1. *Hệ thống hình ảnh minh họa công nghiệp chất lượng cao (`docs/images/`)*: Sinh và tối ưu 4 hình ảnh công nghiệp độ phân giải cao (`oqc_station_setup.jpg`, `oqc_pass_vs_ng_screen.jpg`, `vision_tool_graph_flow.jpg`, `optical_calibration_target.jpg`).
          2. *Bản SOP Thao Tác Chuẩn Kiểm Tra Lấy Mẫu OQC (`docs/sop/SOP_OQC_SAMPLING_INSPECTION.md`)*: Mã hiệu `SOP-OQC-VIS-01`, quy trình 6 bước chuẩn, hướng dẫn chi tiết nhận diện kết quả PASS (xanh lá) vs NG (đỏ thẫm), cách đọc bảng Over Spec ($+\Delta, -\Delta$), quy tắc cách ly khay PASS/NG và xử lý khi $\ge 2$ mẫu NG liên tiếp.
          3. *Bộ Tài Liệu Kỹ Thuật Đào Tạo Kỹ Sư Vision (`docs/training/`)*: Phân rã 4 chuyên đề chuẩn mực dưới 400 dòng/file tuân thủ Rule 5 (Phần 1: Kiến trúc & Giao diện; Phần 2: Camera & Hiệu chuẩn 2D/Chessboard; Phần 3: Lập trình Graph, Origin MvpShapeMatch2, Caliper, CircleFinder, OverSpec, AI OCR; Phần 4: Tích hợp PLC Mitsubishi MC/MX, DB Manager, Lighting RS232/TCP, Đóng gói Job & Cập nhật OTA; cùng tệp Lộ trình đào tạo 4 ngày).
          4. *Cổng Thông Tin Tài Liệu Trung Tâm (`docs/README.md`)*: Mục lục tổng thể và sitemap tra cứu nhanh.
          5. *Tích hợp Tài Liệu Là 1 Phần Của App (Built-in Documentation System)*:
             - Tạo `DocumentationService.cs`: Tự động định vị thư mục `docs/`, chuyển đổi Markdown sang HTML với CSS đẹp mắt thích ứng Dark/Light theme, hiển thị bảng, badges, ghi chú và hình ảnh thẻ local.
             - Tạo `DocumentationViewerWindow.xaml` & `.cs`: Cửa sổ tra cứu tài liệu chuyên nghiệp, sidebar danh mục kèm thanh tìm kiếm nhanh, trình duyệt nội dung HTML `WebBrowser`, nút mở trong Chrome/Edge, mở thư mục đĩa cứng và in ấn / xuất PDF.
          ...
          6. *Kiểm thử tự động*: Xây dựng bộ test `DocumentationSystemTests.cs` (4 test suite toàn diện) kiểm tra định vị thư mục docs, tính toàn vẹn 7 tài liệu, chất lượng hình ảnh và nội dung SOP. Đạt 100% PASSED toàn bộ test suite.


    - [x] **Task 326: Khắc Phục Lỗi Format Huy Hiệu NG Trong Cửa Sổ Hướng Dẫn Sử Dụng (Word Boundary & Bảo Vệ Tiếng Việt / Code Snippets)**:
        - **Hiện tượng**: Trong cửa sổ tài liệu hướng dẫn sử dụng (`DocumentationViewerWindow`), bất kỳ nơi nào có chữ "NG" đều bị bôi đỏ nền trắng, kể cả chữ "NG" nằm bên trong các từ tiếng Việt in hoa thông thường như "THẲNG", "NGANG", "NGUYÊN", "HƯỚNG", "ĐƯỜNG", "KHÔNG", "ĐÚNG", "CÔNG NGHIỆP"...
        - **Nguyên nhân gốc rễ**: Trong `DocumentationService.cs`, phương thức `FormatInline` sử dụng `text.Replace("NG", "<span class='badge-ng'>NG</span>")` thô sơ, thay thế tất cả substring "NG" mà không xét ranh giới từ độc lập và không bảo vệ các khối mã lệnh inline code.
        - **Giải pháp triển khai**:
          1. *Bảo vệ inline code*: Tách và mã hóa an toàn toàn bộ inline code sang placeholder tạm thời kèm `WebUtility.HtmlEncode`, khôi phục sau khi format.
          2. *Định dạng Huy hiệu bằng Word Boundary Regex*: Sử dụng `\bPASS\b`, `\bOK\b`, `\bNG\b`. Trong .NET Regex, toàn bộ các ký tự chữ cái tiếng Việt có dấu (`Ă, Ẳ, Â, Ô, Ơ, Ư, Đ...`) đều thuộc lớp Unicode `\w`. Do đó `\bNG\b` tuyệt đối không bao giờ khớp bên trong các từ như "THẲNG", "NGUYÊN", "HƯỚNG", "ĐƯỜNG", "KHÔNG", "ĐÚNG" hay các từ tiếng Anh như "BYPASS", "PASSWORD", "BOOKING". Chỉ format chính xác các vị trí độc lập (`kết quả NG`, `(NG)`, `PASS/NG`, `| NG |`, `**NG**`...).
          3. *Tối ưu CSS Badge Dark/Light Theme*: Nâng cấp bảng màu cho `.badge-pass` và `.badge-ng` hỗ trợ tương phản cao, đẹp mắt trên cả nền tối và sáng, viền và bo góc tinh tế.
          4. *Kiểm thử tự động*: Bổ sung `TestBadgeWordBoundaryFormatting` vào `DocumentationSystemTests.cs` kiểm tra nghiêm ngặt không có bất kỳ từ tiếng Việt hoặc code snippet nào bị chèn thẻ badge, xác minh 100% các vị trí độc lập được gắn badge chuẩn xác. Toàn bộ test suite đạt 100% PASSED.

    - [x] **Task 327: Nâng Cấp Hệ Thống Hiển Thị Công Thức Toán Học & Chuẩn Hóa Toàn Bộ Ký Hiệu LaTeX Trong Tài Liệu Kỹ Thuật**:
        - **Hiện tượng**: Trong tài liệu hướng dẫn kỹ thuật, nhiều công thức tính toán và biểu thức toán học vẫn hiển thị dạng LaTeX thô chưa được format về dạng dễ đọc (ví dụ `$$\text{PixelsPerMm} = \frac{\text{Khoảng cách tính bằng Pixel}}{\text{Khoảng cách thực tế tính bằng mm}}$$`, `$\rightarrow$`, `$\ge 3$`, `$\le 4\text{ dB}$`, `$\pm 45^\circ$`, `$\Delta X$`, `$10.00\text{ mm}$`...).
        - **Giải pháp triển khai**:
          1. *Nâng cấp Renderer trong `DocumentationService.cs`*: Bổ sung `FormatMathBlock` và `CleanMathExpression`, tự động chuyển đổi các khối công thức `$$...$$` thành thẻ HTML phân số chuẩn công nghiệp (`math-formula-box`, `math-fraction`, `math-numerator`, `math-denominator`) với thiết kế CSS hiện đại cho cả Dark và Light theme; chuyển đổi các biểu thức inline `$...$` thành ký hiệu Unicode trực quan (`➔`, `↔`, `≥`, `≤`, `≈`, `±`, `°`, `×`, `Δ`, `θ`, `…`).
          2. *Chuẩn hóa toàn bộ các tệp Markdown trong `docs/`*: Làm sạch toàn bộ các ký hiệu LaTeX rườm rà trong 5 chuyên đề đào tạo Kỹ sư Vision và bản SOP OQC, đưa về văn phong kỹ thuật tiếng Việt tự nhiên, trong sáng và dễ đọc trực tiếp trên bất kỳ trình xem văn bản nào.
          3. *Kiểm thử & Xác minh*: Bổ sung `TestMathFormulaAndEquationFormatting` vào `DocumentationSystemTests.cs` kiểm tra chính xác các khối phân số và inline math. Đạt 100% PASSED toàn bộ test suite và biên dịch Release 0 lỗi.

    - [x] **Task 328: Khắc Phục Lỗi Hiển Thị Raw Markdown Tại Mục 2.1 Trong Tài Liệu Đào Tạo Kỹ Sư Vision Phần 3 (Unclosed Code Block Bug)**:
        - **Hiện tượng**: Trong cửa sổ tài liệu hướng dẫn sử dụng, khi mở Tài Liệu Đào Tạo Kỹ Sư Vision — Phần 3 (`03_tool_editor_and_inspection_flow.md`), từ mục "### 2.1. So sánh 3 thuật toán Origin cốt lõi" trở xuống toàn bộ nội dung tài liệu bị rơi vào khung code đen và hiển thị dạng markdown raw text.
        - **Nguyên nhân gốc rễ**: Cuối khối ASCII diagram của mục 2.0 bị dính đoạn chuỗi rác kết thúc bằng dấu 3 nháy ngược mở code block (` ``` `) mà không có dấu đóng, dẫn đến cờ `inCodeBlock = true` của trình parse HTML trong `DocumentationService.cs` nuốt toàn bộ nội dung phía sau vào thẻ `<pre><code>`.
        - **Giải pháp triển khai**:
    ...
    - [x] **Task 341: Giải Quyết Triệt Để Vấn Đề Nhắc Nhở & Cài Đặt .NET Desktop Runtime (x86) Cho Module PLC Bridge Chạy Ngầm**:
        - **Mục Tiêu & Yêu Cầu**:
          ...
          3. **Kiểm Thử Toàn Diện**:
             ...
             - Toàn bộ các bộ kiểm thử tự động của dự án PASSED 100% (exit code 0).
             - Toàn bộ Solution `VisionInspectionApp.slnx` biên dịch Release **0 Error(s)**.
    - [x] **Task 342: Khắc Phục Triệt Để Lỗi Bất Đồng Bộ Số Lượng Điểm Object và Image (numberOfObjectPoints == numberOfImagePoints) Khi Calibrate Camera Bàn Cờ**:
        - **Mục Tiêu & Yêu Cầu**:
          1. **Khắc phục ngoại lệ unhandled OpenCVException khi bấm Calibrate Camera**:
             - Hiện tượng: Sau khi thêm các ảnh bàn cờ đã nhận diện được, bấm Calibrate Camera thì chương trình bị phát sinh lỗi ngoại lệ:
             ...
             - Nguyên nhân gốc rễ:
               + Người dùng nhập số ô cờ 8x6 (48 ô), nhưng trên bàn cờ phẳng thực tế chỉ có (8 - 1) x (6 - 1) = 7 x 5 = 35 góc trong (inner corners).
               + Thuật toán đa chiến lược `DetectCornersMultiStrategy` đã tự động tìm thấy 35 góc (7x5) qua cờ `tryAlternativeConvention = true`, nhưng `ChessboardCaptureItem` không lưu lại kích thước phát hiện được (`DetectedPatternSize`).
               + Khi bấm Calibrate, `RunCalibrate` dùng kích thước cấu hình tĩnh 8x6 (48 điểm) để tạo `objPointsTemplate`, trong khi các ảnh trong danh sách lại có 35 điểm `imagePoints`.
               + `Cv2.CalibrateCamera` kiểm tra số lượng điểm của từng ảnh và ném ra ngoại lệ `OpenCVException` do không khớp số điểm giữa object points và image points.
               + Cả tầng Service lẫn ViewModel đều chưa có `try...catch (OpenCVException)` bảo vệ.
          2. **Kiến Trúc & Giải Pháp Kỹ Thuật Đã Triển Khai**:
             - **Lưu Trữ Pattern Size Thực Tế Từng Ảnh (`ChessboardCaptureItem` & `ChessboardCalibrationViewModel`)**:
               + Bổ sung thuộc tính `public OpenCvSharp.Size DetectedPatternSize { get; init; }` vào `ChessboardCaptureItem`.
               + Cập nhật `StatusText` hiển thị chi tiết số góc và kích thước lưới (ví dụ: `✅ 35 corners (7×5)`).
               + Cập nhật `_lastDetection` thành bộ 3 `(bool Found, Point2f[] Corners, Size PatternSize)`.
               + Cập nhật cả `AddCapture()` và `SnapAndAddAsync()` để ghi nhớ chính xác `DetectedPatternSize = result.PatternSize`.
             - **Thuật Toán Suy Luận Kích Thước & Khởi Tạo Object Points Theo Từng Ảnh (`Per-View Object Points`)**:
               + Xây dựng hàm `InferPatternSize(int cornerCount, Size hintPatternSize)` trong `ChessboardCalibrationService`: tự động giải mã các quy ước ô vuông trừ 1 (8x6 ô -> 7x5 góc), chiều xoay đảo ngược 90 độ (WxH vs HxW), hoặc các cặp thừa số hình học gần nhất.
               + Nâng cấp `Calibrate` hỗ trợ `perViewPatternSizes`: sinh `objectPoints` tương ứng chính xác 100% cho từng ảnh dựa trên kích thước lưới của ảnh đó, đảm bảo bất biến `objectPoints[i].Count == imagePoints[i].Count` triệt tiêu 100% lỗi mismatch OpenCV.
               + Cơ chế phân nhóm và lọc an toàn: Nếu danh sách ảnh có các ảnh lệch số góc (ví dụ 4 ảnh 35 góc và 1 ảnh 48 góc), tự động lọc nhóm chiếm đa số (dominant group >= 3 ảnh) để calibrate thành công mà không gây crash.
               + Bọc `Cv2.CalibrateCamera` trong khối `try...catch (OpenCvSharp.OpenCVException)` và `catch (Exception)`, trả về kết quả an toàn kèm `ErrorMessage` rõ ràng.
             - **Tự Động Đồng Bộ Cấu Hình & Giao Diện Người Dùng (`RunCalibrate`)**:
               + Tự động nhận diện khi tất cả ảnh chụp là 35 góc trong khi UI đang để 8x6, tự động chuyển `PatternConvention` sang `SquareCount` (hoặc đồng bộ `BoardCols / BoardRows`), giúp thông số lưu vào `ChessboardCalibrationData` hoàn toàn nhất quán.
               + Báo cáo trực quan cho người dùng nếu có ảnh bị loại bỏ do lệch góc.
               + Bọc toàn bộ quá trình calibrate trong `try...catch` hiển thị lỗi trên `StatusMessage`, không bao giờ làm gián đoạn ứng dụng.
          3. **Kiểm Thử Toàn Diện**:
             - Xây dựng bộ test tự động mới `TestExtractApp/ChessboardCalibrationMismatchTests.cs` (5 bài kiểm thử):
               * Test 1: Kiểm tra `InferPatternSize` suy luận chính xác các quy ước kích thước và chiều xoay.
               * Test 2: Kiểm tra Calibrate thành công khi UI nhập 48 góc (8x6) nhưng ảnh thực tế có 35 góc (7x5) - không crash, tính đúng ma trận camera và sai số reprojection.
               * Test 3: Kiểm tra tự động lọc ảnh lệch góc khi có tập ảnh hỗn hợp (dominant group >= 3).
               * Test 4: Kiểm tra Calibrate với ảnh xoay 90 độ (7x5 và 5x7 cùng 35 góc) với per-view object points.
    ...
    - [x] **Task 350: Khắc Phục Lỗi Caliper Bị Dịch Lệch Khi Bật Undistort (ImageSource) & Đồng Bộ Toàn Bộ Pipeline Các Tool**:
        - **Hiện Tượng & Yêu Cầu Người Dùng**:
